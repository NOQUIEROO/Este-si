...
def _resolve_salt(data_dir: Path) -> bytes:
    """La sal del HMAC de miembros. Vive fuera de la base a proposito."""
...
class Config:
    ...
    def from_env(cls, dotenv: Path | None = Path(".env")) -> "Config":
        ...
        if not token:
            raise SystemExit(
                "Falta BOT_TOKEN. Copia .env.example a .env y pone el token de @BotFather."
            )
        ...
        return cls(
            ...
            admin_ids=admin_ids,
            scan_radius_m=_int_env("SCAN_RADIUS_M", 3000),
            scan_limit=_int_env("SCAN_LIMIT", 5),
